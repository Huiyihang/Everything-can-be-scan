...
现实物品的 AR 奇遇记，一个移动端优先的 H5 PoC。用户拍摄或上传一张日常物品照片后，应用调用视觉大模型识别主要物品，并以物品第一人称生成一篇 趣味日记，同时给出一个相关服务推荐和淘宝移动端搜索入口。

项目核心目标是把“扫一扫”扩展成探索现实世界的趣味入口：杯子、鞋子、台灯、植物、键盘都可以拥有自己的小情绪和下一步服务建议。
...
- 支持 Vitest 自动化测试，覆盖推荐策略、日记改写和扫描 API 关键分支

## 项目亮点

- 完整业务闭环：覆盖拍照/上传、图片压缩、AI 识别、结果生成、服务推荐、淘宝跳转和历史记录，不只是单页演示。
- 移动端优先体验：针对手机拍照、局域网真机访问、结果页刷新保留、分享图生成等真实使用场景做了设计。
- 稳定演示能力：提供 Mock 模式、请求超时、错误提示和重试入口，避免外部模型或网络波动影响演示。
- 结构化 AI 接入：后端统一调用视觉大模型，并解析结构化 JSON，便于后续扩展更多物品属性和服务策略。
- 推荐策略可扩展：按物品类别、状态和使用场景映射推荐服务，为接入真实商品、优惠券或服务平台预留空间。
- 工程化基础完整：使用 Next.js App Router、TypeScript、Tailwind、环境变量模板、PWA manifest、Service Worker 缓存策略和清晰的模块拆分。
- 可验证交付：内置自动化测试、类型检查和生产构建命令，降低演示前临时故障和回归风险。
...
<img src="./public/screenshots/lishi.png" alt="万物皆可扫推荐历史界面" width="320" height="640" />
